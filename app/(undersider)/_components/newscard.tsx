...
import { ExternalLink, Eye, EyeOff, Loader2, Pencil, Trash2, Newspaper, ArrowRight } from 'lucide-react';
...
export function NewsCard({
  ...
  return (
    <TooltipProvider delayDuration={100}>
      <Card className={cn(
        "flex flex-col overflow-hidden transition-all duration-300 group border h-full relative",
        ...
        <CardFooter className="flex justify-between items-center p-3 bg-muted/30 dark:bg-slate-800/30 border-t">
          <Link 
            href={linkHref}
            className={cn(
              buttonVariants({ variant: "link", size: "sm" }),
              "text-primary hover:text-primary/90 text-sm font-medium"
            )}
          >
            Les mer
