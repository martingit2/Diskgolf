// Fil: /components/BaneCarousel.tsx
// Formål: Viser en karusell med topprangerte discgolf-baner. Håndterer henting av banedata, favorittstatus, lasting og feiltilstander.
...
import useCarouselStore from "@/app/stores/useCarosuellStore"; // Store for karuselldata (her: topprangerte baner).
...
const BaneCarousel = () => {
  ...
  // Henter topprangerte baner hvis listen er tom og vi ikke laster eller har feil.
  useEffect(() => {
  ...
  }, [fetchTopCourses, topCourses.length, loadingCourses, errorCourses]);
  ...
  const renderSkeletons = () => {
    ...
    return (
      <Swiper
        key="bane-carousel-skeleton-swiper" // Unik nøkkel for skeleton Swiper.
      ...
      <div className="flex justify-between items-end mb-4">
        ...
        <div>
           <h2 className="text-sm font-semibold text-green-600 uppercase tracking-wider">
             {t('popular_courses_carousel.header.subtitle')} {/* Oversatt undertittel. */}
           </h2>
           <h1 className="text-3xl font-extrabold text-gray-900 leading-tight mt-1">
             {t('popular_courses_carousel.header.title')} {/* Oversatt hovedtittel. */}
           </h1>
        </div>
        ...
        >
          {t('popular_courses_carousel.header.view_all_button')} {/* Oversatt knappetekst. */}
 ...
 // -----------------------------
    ...
    return (
      <section className="max-w-7xl mx-auto p-6 mt-20">
        ...
        <div className="mt-8 text-center text-red-600">
            ...
            <p>{t('popular_courses_carousel.error.load_failed', { error: errorCourses })}</p>
        </div>
      </section>
    );
  }
  ...
  // Viser en melding hvis lasting er ferdig, ingen feil oppstod, men ingen baner ble funnet.
   if (!loadingCourses && !favoritesInitializing && topCourses.length === 0 && !errorCourses) {
    return (
      <section className="max-w-7xl mx-auto p-6 mt-20">
        {renderHeader()} {/* Viser headeren. */}
        <p className="text-gray-600 mt-8 text-center">
            {t('popular_courses_carousel.empty.message')} {/* Viser oversatt melding. */}
        </p>
      </section>
    );
  }
    ...
    <section className="max-w-7xl mx-auto p-6 mt-20">
      {renderHeader()} {/* Rendrer headeren. */}
       ...
       {isLoading ? (
         ...
         <Swiper
           key="bane-carousel-real-swiper" // Unik nøkkel for å tvinge re-mount etter lasting.
           modules={[Navigation, Pagination, Autoplay]}
           autoplay={{ delay: 5000, disableOnInteraction: false }} // Autoplay-innstillinger.
           pagination={{
             clickable: true,
         ...
         >
           {/* Mapper gjennom de hentede topprangerte banene. */}
