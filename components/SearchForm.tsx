...
const formSchema = z.object({
  ...
  difficulty: z.string().optional(), // Bruker SHOW_ALL_VALUE internt
});
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      const filteredData = data.filter((course) => {
        ...
        // Sjekker mot den interne SHOW_ALL_VALUE eller den faktiske verdien.
        const locationMatches = !values.location || values.location === SHOW_ALL_VALUE || formattedLocation === values.location;
        const difficultyMatches = !values.difficulty || values.difficulty === SHOW_ALL_VALUE || course.difficulty === t(`search_form.difficulty_${values.difficulty?.toLowerCase()}`); // Må oversette for sammenligning hvis difficulty lagres oversatt
      ...
      const formattedResults = filteredData.map((course) => ({
        ...
        description: course.description ?? t('common.no_description', 'Ingen beskrivelse tilgjengelig.'), // Eksempel på fallback med t()
                ...
                <FormControl>
                   <Select onValueChange={field.onChange} value={field.value ?? SHOW_ALL_VALUE}>
                    ...
                    <SelectContent>
                      ...
                      {/* Bruker key som intern verdi, viser oversatt tekst */}
                      <SelectItem value="Lett">{t('search_form.difficulty_easy')}</SelectItem>
                      <SelectItem value="Middels">{t('search_form.difficulty_medium')}</SelectItem>
                      <SelectItem value="Vanskelig">{t('search_form.difficulty_hard')}</SelectItem>
                    </SelectContent>
                  </Select>
                </FormControl>
                <FormMessage />
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {searchResults.map((course) => (
              // Sender props videre til CourseCard. Antar CourseCard håndterer i18n internt.
              <CourseCard
                key={course.id}
                course={course}
